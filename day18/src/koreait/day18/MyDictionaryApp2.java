...
public class MyDictionaryApp2 {
	// 전역변수 선언
	static Scanner sc = new Scanner(System.in);
	static List<Word> mywords = new ArrayList<>();

	public static void main(String[] args) { // 애플리케이션 클래스
		JFrame f = new JFrame();
		FileDialog fd;
		boolean run = true;
		while (run) {
	...
	private static void wordSave(String filename) {// 파일에 단어들을 저장합니다.
	...
	private static void wordRead(String filename) {// 파일에서 단어들을 읽어옵니다.
		File file = new File(filename);
