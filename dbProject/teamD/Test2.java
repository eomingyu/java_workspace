...
import javax.swing.event.ChangeListener;
	...
	public class Test2 {
		private JFrame frame;															// 메인 Frame
		private JFrame countF;															// 결제 시 예/아니요 묻는 Frame
		private JFrame cart;
		List<String> MS = new ArrayList<>();
		   ...
		   String btnString1_1 = "";													// menu[0]에 대한 문자열 합계를 담는 변수
		   String btnString2_1 = "";													// menu[1]에 대한 문자열 합계를 담는 변수
		   String btnString3_1 = "";													// menu[2]에 대한 문자열 합계를 담는 변수
		   String btnString4_1 = "";													// menu[3]에 대한 문자열 합계를 담는 변수
		   
		   String btnString1_2 = "";													// menu[0]에 대한 문자열 합계를 담는 변수
		   String btnString2_2 = "";													// menu[1]에 대한 문자열 합계를 담는 변수
		   String btnString3_2 = "";													// menu[2]에 대한 문자열 합계를 담는 변수
		   String btnString4_2 = "";													// menu[3]에 대한 문자열 합계를 담는 변수
		...
		private void initialize() {
		    ...
		    btnconfirm.setBounds(470, 870, 125, 60);
		    ...
		    lbltotalP.setBounds(435, 750, 142, 37);
		    ...
		    textArea.setBounds(0, 0, 650, 700);
		    textArea.setEditable(false);
		    textArea.setText("제품명\t\t수량\t가격\n\n");
		    textArea.setFont(new Font("기본", Font.ITALIC, 27));
		    panel_cart.add(textArea);														// 장바구니 관련 설정 시작
		    
		    JLabel lab_total = new JLabel("총 결제금액");
		    lab_total.setBounds(0, 830, 650, 790);
		    lab_total.setFont(new Font("굴림", Font.BOLD, 30));
		    lab_total.setBounds(35,750,180,37);
		    panel_cart.add(lab_total);
		      ...
		      spinner1.addChangeListener(new ChangeListener() {				//menu에 spinner1 설정 끝
		         ...
		         public void stateChanged(ChangeEvent e) {            		//menu에 spinner5 클릭시 이벤트 발생 시작
		            if((int)spinner1.getValue()>0) {
		            	btnString1 = menu[0]+"\t\t"+(int)spinner1.getValue()+"\t"+textLavel(cost[0])+"\n\n\n";  
		            } else {
		            	btnString1 = "";
		            }
		      ...
		      spinner2.addChangeListener(new ChangeListener() {					//menu에 spinner2설정 끝
		         ...
		         public void stateChanged(ChangeEvent e) {					//menu에 spinner5 클릭시 이벤트 발생 시작
		            if((int)spinner2.getValue()>0) {
		            	btnString2 = menu[1]+"\t\t"+(int)spinner2.getValue()+"\t"+textLavel(cost[1])+"\n\n\n";  
		            } else {
		            	btnString2 = "";
		            }
		         }
		      ...
		      spinner3.addChangeListener(new ChangeListener() {				//menu에 spinner3 설정 끝
		         ...
		         public void stateChanged(ChangeEvent e) {					//menu에 spinner5 클릭시 이벤트 발생 시작
		            if((int)spinner3.getValue()>0) {
		            	btnString3 = menu[2]+"\t\t"+(int)spinner3.getValue()+"\t"+textLavel(cost[2])+"\n\n\n";  
		            } else {
		            	btnString3 = "";
		            }
		      ...
		      spinner4.addChangeListener(new ChangeListener() {				//menu에 spinner4 설정 끝
		         ...
		         public void stateChanged(ChangeEvent e) {						//menu에 spinner5 클릭시 이벤트 발생 시작
		            if((int)spinner4.getValue()>0) {
		            	btnString4 = menu[3]+"\t\t"+(int)spinner4.getValue()+"\t"+textLavel(cost[3])+"\n\n\n";  
		            } else {
		            	btnString4 = "";
		            }
		         }
		      ...
		      spinner5.addChangeListener(new ChangeListener() {					//menu에 spinner5 클릭시 이벤트 발생 시작
		         ...
		         public void stateChanged(ChangeEvent e) {
		            if((int)spinner5.getValue()>0) {
		            	btnString5 = menu[4]+"\t\t"+(int)spinner5.getValue()+"\t"+textLavel(cost[4])+"\n\n\n";  
		            } else {
		            	btnString5 = "";
		         }
		        }
...
//		             spinner_sizeup.setValue(0);
		      ...
		      btnClean.setBounds(80, 870, 125, 60);							// 취소 하기 버튼 
