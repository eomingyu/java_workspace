...
//전체 select	
	public List<Order> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				orders.add(new Order(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getInt(4), rs.getDate(5)));
			}
			pstmt.close();
...
// 1개 주문번호 select
	public Order selectOne(int order_num) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				vo = new Order(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getInt(4), rs.getDate(5));
			}
			pstmt.close();
