...
public class MathTest {
	public static void main(String[] args) {
		...
		//소수 판별 수식을 생각해봅시다. 정수 num 이 소수일까? 
		//	2부터 num-1까지 나누어서 나머지가 0인 값이 있으면 소수 아님.
		//	->* num이 15일 때 2에서 14까지 나누어서 나머지가 0이 있는지? ->*** 2 ~ 루트(제곱근) 값까지 나누면 됨.
		//	->** 제급근 대신에 2부터 num/2(몫)까지 나누어서 할 수도 있습니다.		ㄴ>큰 값 판단 시 실행 속도 개선
		...
		int num = 21;	//테스트 값은 15,17,19,21
		boolean isPrime = true;
		
		for(int i=2;i<Math.sqrt(num);i++) {	//조건은 * i<num , **i<num/2 , *** i<Math.sqrt(num)
				//num이 소수일 때 반복횟수가 가장 작은 조건식은? *** i<Math.sqrt(num) 입니다.
