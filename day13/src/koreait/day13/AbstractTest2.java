...
public class AbstractTest2 {
	public static void main(String[] args) {
		//추상클래스 타입으로 배열을 선언하고 자식 클래스 객체를 참조하는 예제입니다.
		AShape[] shapes = new AShape[3];		//ArrayList 사용도 가능합니다.
		//쓰는 이유 : shapes 동일한 배열에 서로 다른 자식 객체를 참조하게 됩니다.
		shapes[0] = new ATriangle();
		shapes[1] = new ASquare();
		shapes[2] = new ATriangle();
		
//		ATriangle ty = (ATriangle)shapes[1];	//오류 발생 : 만들어진 객체와 다른 타입으로 캐스팅 못합니다.
		ATriangle ty = (ATriangle)shapes[2];	//정상 실행 : 만들어진 객체와 참조타입이 일치합니다.
