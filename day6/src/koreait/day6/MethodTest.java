...
public class MethodTest {
	...
	public static void main(String[] args) {
		...
		//메소드 실행 - 메소드 이름 호출합니다.
		testMethod1();		//main 메소드의 특징: static, 실행할 수 있는 메소드는 static만 가능합니다.
		testMethod1(); 		//-> static은 static 성질을 갖는 요소들만 사용합니다.

		//		testMethod2(123, 123);	//오류-> 두 번째 인자의 형식이 틀림.
		testMethod2(123, "twice");
		testMethod2(456, "momo");
		testMethod22(1.34, 23, 4.56);	//3개의 인자값을 더해서 출력하는 메소드입니다.
		testMethod22(23, 45, 1.8987);		//인자값 전달할 때 자동캐스팅 됩니다.

		int result = testMethod3();				//반환값(리턴값)이 있는 메소드는 변수를 사용하여 저장 또는 출력합니다.
		...
		System.out.println("testMethod4 return = " + testMethod4(3, 42));
		//testMethod44를 실행해서 리턴값을 변수 d_result에 저장하고 출력하세요
		double d_result = testMethod44(3.32, 42);
		System.out.println("testMethod44 return = " + d_result);

		System.out.println(dayOfWeek(6));
		System.out.println(dayOfWeek2(11));		//반환 받은 문자열의 참조값으로 내용을 출력.
	...
	//2.인자 있고, 반환값이 없는 메소드입니다.
	//인자는 개수 제한이 없습니다. 변수 선언과 같은 방식으로 , 로 구분합니다.
	//인자는 메소드 실행에 필요한 데이터입니다.
	...
	//3. 인자 없고, 반환값이 있는(***반환되는 데이터의 형식***) 메소드입니다.
	static int testMethod3() {
		return 0;		//*반환값이 있는 형식에서는 꼭 있어야하는 명령문 -> return
	...
	//정수값을 전달 받습니다. 1이면 "월요일, 2면  "화요일", 3이면 수요일,....7이면 일요일을 리턴하는 메소드
	// 1~7 이외의 값은 "unknown"을 리턴하는 메소드 -> 메소드 이름은 weekday
	static String dayOfWeek(int a) {
		...
		if(a==1)
			...
			result="unknown";
		return result;			//result는 문자열의 참조값(메모리의 주소)를 반환합니다.
	}
	//switch문, 리턴값은 문자열 변수
	static String dayOfWeek2(int a) {
