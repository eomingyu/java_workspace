...
// StudentScore 클래스의 배열을 연습합니다.
	public static void main(String[] args) {
		...
		// 입력&배열 저장된 객체에서 name 필드만 모두 출력해 봅니다.
		System.out.println("객체 생성하고 필드값은 없는 경우: ");
		iclass[4].printScore();
		System.out.print("학생명단: ");
		for (int i = 0; i < iclass.length; i++) {
		...
		for (int i = 0; i < iclass.length; i++) {
//		System.out.printf("\n학생이름: %s - 총점: %d , 평균: %.2f",iclass[i].name, iclass[i].sum(), iclass[i].avg());
			DecimalFormat df = new DecimalFormat("###.##");	//출력형식을 지정합니다. #하나가 1자리수 //"###.00"하면 정수 뒤에 .00
			String avg = df.format(iclass[i].avg());		//출력형식을 적용하여 문자열로 변환합니다.
			
			System.out.println(iclass[i].name + "- 총점: " + 
							iclass[i].sum() + ", 평균:" + avg);
		}
		// printScore()메소드로 출력
		for (int i = 0; i < iclass.length; i++)
